# Pykinect2 Test Code for Python 3.10 with Face Recognition and Body Tracking
import cv2
import numpy as np
import time
...
# Kalman filter for tracking body position when face is not visible
...
while True:
    ...
    if kinect.has_new_depth_frame() and not face_detected:
        ...
        if np.any(frame):
            ...
            # If face is not detected, use Kalman filter to track the body
            if current_position is not None:
                current_position, _ = kf.filter_update(filtered_state_mean=current_position, filtered_state_covariance=np.eye(2), observation=current_position)
                cv2.circle(depth_image, (int(current_position[0]), int(current_position[1])), 10, (255, 0, 0), -1)
